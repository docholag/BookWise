...
function Digit({ value, place }: Readonly<{ value: number; place: number }>) {
  ...
  return (
    <div className="relative inline-block w-[1ch] overflow-y-clip overflow-x-visible tabular-nums leading-none">
      ...
      {Array.from({ length: 10 }, (_, i) => (
        <NumberCount key={i} mv={animatedValue} number={i} />
      ))}
    </div>
  );
}
...
function NumberCount({
  mv,
  number,
}: Readonly<{ mv: MotionValue<number>; number: number }>) {
